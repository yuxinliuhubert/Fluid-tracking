function [r0]= proj2r0_lin(proj,theta,SRD,RDD,delta_T) %takes in any N (even #) by 2 of projection coordinates and the angle 
...
%% Now, we expand the number of columns to incorporate new variables: u, v , w, ax,ay,az
...
%            A( 2*j-1,new_col_num)=0.5*(delta_T*(j-1))^2;%for w term 
     ...
     for k=2:NOS  
         ...
         % transformed and the first shot
              A(IoR, new_col_num-2)=cos(theta*NoT)*delta_T*(NoT);
              A(IoR+1, new_col_num-2)=-sin(theta*NoT)*delta_T*(NoT);%for u
              A(IoR, new_col_num-1)=sin(theta*NoT)*delta_T*(NoT);
              A(IoR+1, new_col_num-1)=cos(theta*NoT)*delta_T*(NoT);%for v
