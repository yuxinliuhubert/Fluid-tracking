function [r0]= proj2r0_acc_combination(xz_proj,theta,SOD,RDD,delta_T) %takes in any N (even #) by 2 of projection coordinates and the angle 
   ...
   %position before camera machine starts rotating
    NOS=height(xz_proj); SDD=(SOD+RDD);
...
%only z-directions needs such correction
     ...
     for k = 2:(NOS)
         ...
         for i=1:trans_count %index i corresponds to the index for the position being transformed to k
             theta_multi=theta*i; %theta_multi stands for multiples of theta, or number of theta's 
             T1=T2-i*delta_T;
              A(IoR, u_ind)=cos(theta_multi)*(T2-T1);
              A(IoR+1, u_ind)=-sin(theta_multi)*(T2-T1);%for u
              A(IoR, v_ind)=sin(theta_multi)*(T2-T1);
              A(IoR+1, v_ind)=cos(theta_multi)*(T2-T1);%for v
