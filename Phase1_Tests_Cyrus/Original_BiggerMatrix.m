function [percent_error]=Original_BiggerMatrix(true_position_3d,noise,NOS, theta_degree)
%% Data Input
format long
 ...
 %% function for rotating coordinate axes by a given angle alpha 
   ...
   r0=zeros(height(proj)/2,3); %initialize the predicted positions
       for i = 1:(height(proj)/2)
           ...
           zi_1=proj(2*i-1,2); zi_2=proj(2*i,2);
           A=[cos(theta), sin(theta), -1, 0 0; 
                 -sin(theta), cos(theta), 0, -1 0; 
                 0, 0, -1, xi_2/(SRD+RDD),0;
                 -1, xi_1/(SRD+RDD), 0, 0, 0;
                 0, -zi_1/(SRD+RDD),0,0,1;
                 0,0,0,-zi_2/(SRD+RDD),1];
